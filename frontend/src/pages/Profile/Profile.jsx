...
const ProfilePage = () => {
  ...
  const sendMessage = () => {
    ...
    if (activeTab === "pending") {
      setPendingRequests((prev) =>
        prev.map((req) =>
          req.id === activeChat
            ? { ...req, chatMessages: [...req.chatMessages, message] }
            : req
        )
      );
    } else {
  ...
  const getActiveRequest = () => {
    if (activeTab === "pending") {
      return pendingRequests.find((req) => req.id === activeChat);
    } else {
      return completedRequests.find((req) => req.id === activeChat);
    }
  };
  ...
  const RequestCard = ({ request, type = "pending" }) => (
    <div
      className={`border rounded-xl p-5 mb-4 transition-all hover:shadow-md cursor-pointer ${
        activeChat === request.id ? "ring-2 ring-indigo-500" : ""
      } ${
        type === "completed"
          ? request.status === "accepted"
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              <div className={`${activeChat ? "lg:w-2/3" : "w-full"}`}>
                <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
              ...
              {/* Chat Panel */}
              <div className={`${activeChat ? "lg:w-1/3" : "hidden"}`}>
                <ChatPanel />
              </div>
            </div>
          </div>
